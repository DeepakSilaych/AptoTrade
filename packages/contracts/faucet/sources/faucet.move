module exchange_faucet::faucet {
    ...
    /// Permissionlessly burn a coin type to a caller's coin store.
    public entry fun burn<CoinType>(
        account: &signer,
        amount: u64,
    ) acquires CapabilityStore {
        // check amount less than balance
        ...
        // Immutably borrow the burn capability for the coin type.
        let burn_cap_ref = &borrow_global<CapabilityStore<CoinType>>(
            coin_type_publisher_address
        ).burn_cap;

        //  Burn the withdrawn coins

        let to_burn = coin::withdraw(account, amount);
        coin::burn(to_burn, burn_cap_ref);
    }
